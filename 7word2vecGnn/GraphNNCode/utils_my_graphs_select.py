...
def generate_epoch_valid(Gs, M):
    ...
    while st < len(Gs[0]):
        ...
        mask = Gs[1][st:st+M,:]
        y = Gs[2][st:st+M, :]
        filename= Gs[3][st:st+M]
        # X, mask, y = get_pair_my(Gs, M, st=st)  # X 是特征矩阵，m 是邻接矩阵，y 是label
        epoch_data.append((X, mask, y,filename))
        st += M
    return epoch_data
...
# 计算模型的准确度
def get_auc_epoch_test(model, graphs, batch_size, train_str,dist_bad_str,dist_good_str,load_data=None, valortest=False):
    ...
    for cur_data in epoch_data:
        ...
        #记录测试集中预测为1的文件名
        if valortest:
            for predict_label,true_label,file in zip(result,label,filename):
                # if predict_label == true_label:
                    if predict_label == 0:
                        copyfile(train_str + '/' + file, dist_good_str+ '/' + file)
                    elif predict_label == 1:
                        copyfile(train_str + '/' + file, dist_bad_str + '/' + file)
        # a_result_1=model.calc_result(X, m)
        results.extend(result)
        labels.extend(label)
        #操作filename就ok了，把result==1位置的filename输出就好

    # workbook = xlsxwriter.Workbook('data.xlsx')  # 创建一个Excel文件
...
def get_auc_epoch_test_new(model, graphs, batch_size, load_data=None):
    ...
    for cur_data in epoch_data:
        ...
        r = i*l + r
        l = len(x1)
        worksheet.write_column(r, 0, x1)
        worksheet.write_column(r, 1, y1)
        worksheet.write_column(r, 2, la)
        i = i + 1
