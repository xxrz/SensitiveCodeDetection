...
class GadgetVectorizer:
    ...
    def vectorize(self, gadget):
        num=100
        tokenized_gadget, backwards_slice = GadgetVectorizer.tokenize_gadget(gadget)
        vectors = numpy.zeros(shape=(num, self.vector_length))#数字代表要多少个词组
        try:
            if backwards_slice:
                for i in range(min(len(tokenized_gadget), num)):
                    vectors[num - 1 - i] = self.embeddings[tokenized_gadget[len(tokenized_gadget) - 1 - i]]
            else:
                for i in range(min(len(tokenized_gadget), num)):
    ...
    def train_model(self):
        ...
        # model = Word2Vec(self.gadgets, min_count=1, size=self.vector_length, sg=1, window=5, negative=3, sample=0.001, hs=1, workers=4)
        # model.save("./" + "word2vec.pkl")
        model = Word2Vec.load("./" +"word2vec.pkl")
